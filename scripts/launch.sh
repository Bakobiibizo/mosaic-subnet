...
burn_fee=2.5
source_miner="mosaic_subnet/miner/__init__.py"
source_validator="mosaic_subnet/validator/__init__.py"

# Configures the module launch
configure_launch() {
    # Enter the path of the module
    echo "The module path in the format of \"filename.ClassName\" (eg. eden.Miner)"
    ...
    if [ "$is_miner" = "true" ]; then
        if [ ! -f "mosaic_subnet/miner/$filename.py" ]; then
            # Copy the source miner file to the destination
            destination_file="mosaic_subnet/miner/$filename.py"
            cp "$source_miner" "$destination_file"
            # Replace the class name in the destination file
            sed -i "s/Miner/$classname/g" "$destination_file"
            echo "Miner module created at mosaic_subnet/miner/$filename.py"
        fi
    fi

    # Create the validator module if it doesn't exist
    if [ "$is_validator" = "true" ]; then
        if [ ! -f "mosaic_subnet/validator/$filename.py" ]; then
            # Copy the source validator file to the destination
            destination_file="mosaic_subnet/validator/$filename.py"
            cp "$source_validator" "$destination_file"
            # Replace the class name in the destination file
            sed -i "s/Validator/$classname/g" "$destination_file"
            echo "Validator module created at mosaic_subnet/validator/$filename.py"
        fi
    fi
    ...
    if [ "$needs_stake" = "true" ]; then
        ...
        echo "Validators require 5202.5 com to cover the burn fee and have enough balance to vote."
        echo "Miners require 258.5 to cover the burn fee to mine."
        # shellcheck disable=SC2162
...
create_key() {
    ...
    # shellcheck disable=SC2162
    read -p "Display key details? (y/n): " display_key
    if [ "$display_key" = "y" ]; then
...
transfer_balance() {
    ...
    echo "So you need to send 102.5 to send 100 tokens."
    # shellcheck disable=SC2162
...
serve_miner() {
    echo "Serving Miner"
    pm2 start "python -m mosaic_subnet.cli miner $module_path $key_name $host $port"
    echo "Miner served."
}
...
register_miner() {
    ...
    comx module register "$module_path" "$key_name" --netuid "$netuid" --stake "$stake" --ip "$ip_address" --port $"$port"
...
serve_validator() {
    ...
    pm2 start "python -m mosaic_subnet.cli miner $key_name $module_path $host $port"
